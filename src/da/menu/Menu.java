...
import da.menu.mode.LightDarkMode;
...
import da.menu.mode.ToolBarAccentColor;
...
public class Menu extends JPanel {
    ...
    public Menu(String userEmail, String userName) {
    ...
    public void setMenuFull(boolean menuFull) {
        ...
        if (menuFull) {
            header.setText(userName); // Sử dụng tên người dùng
    ...
    private void init() {
        ...
        header = new JLabel(userName); // Hiển thị tên người dùng

        // Load và thay đổi kích thước ảnh
        ImageIcon icon = new ImageIcon(getClass().getResource("/da/icon/png/user.png"));
    ...
    public void createMenu() {
        DanhMucChucNangService service = new DanhMucChucNangService();
        int idVaiTro = service.getCurrentUserRole(userEmail);
        List<MenuItem> danhMuc = service.layDanhMucChucNang(this, idVaiTro);
        for (MenuItem item : danhMuc) {
            panelMenu.add(item);
        }
    }
    ...
    private class MenuLayout implements LayoutManager {
        ...
        public void layoutContainer(Container parent) {
            synchronized (parent.getTreeLock()) {
                ...
                int headerFullHgap = UIScale.scale(Menu.this.headerFullHgap);
                ...
                header.setBounds(x + headerFullHgap, y, width - (headerFullHgap * 2), headerHeight);
                ...
                int menux = x;
                int menuy = y + headerHeight + gap;
                int menuWidth = width;
                int menuHeight = height - (headerHeight + gap) - (ldHeight + ldgap * 2) - accentColorHeight;
